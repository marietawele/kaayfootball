...
export class AddTypeReservationComponent {
  ...
  init_form() {
    this.reactiveForm_add_type_reservation = this.formBuilder.group({
      libelle_type_reservation: ['', Validators.required],
      description: [''],
  ...
  get f(): any {
    return this.reactiveForm_add_type_reservation.controls;
  }
  ...
  onSubmit_add_type_reservation() {
    ...
    type_reservation.id_entreprise = this.api.current_entreprise.id_entreprise;
    type_reservation.id_utilisateur = this.api.current_entreprise.id_utilisateur;
  ...
  add_type_reservation(type_reservation: any) {
    ...
    this.api.taf_post(
      'type_reservation/add',
      type_reservation,
      (reponse: any) => {
        this.loading_add_type_reservation = false;
        if (reponse.status) {
          console.log(
            'Opération effectuée avec succés sur la table type_reservation. Réponse= ',
            reponse
          );
          ...
          console.log(
            "L'opération sur la table type_reservation a échoué. Réponse= ",
            reponse
          );
  ...
  get_details_add_type_reservation_form() {
    ...
    this.api.taf_post(
      'type_reservation/get_form_details',
      {},
      (reponse: any) => {
        if (reponse.status) {
